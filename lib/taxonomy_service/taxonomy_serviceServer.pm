package taxonomy_service::taxonomy_serviceServer;
...
sub call_method {
    ...
    my $ctx = taxonomy_service::taxonomy_serviceServerContext->new($self->{loggers}->{userlog},
...
{
    # Service taxonomy_service requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for taxonomy_service but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = taxonomy_service::taxonomy_serviceServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service taxonomy_service.");
    }
...
package taxonomy_service::taxonomy_serviceServerContext;

use strict;

=head1 NAME

taxonomy_service::taxonomy_serviceServerContext
...
package taxonomy_service::taxonomy_serviceServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = taxonomy_service::taxonomy_serviceServer->new(
